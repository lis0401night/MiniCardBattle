...
async function main() {
  ...
  // 1. 新水着通常アイコンのデータを取得
  const normalImg = sharp('public/assets/icons/icon_dragon_summer.webp');
  const normalRaw = await normalImg.raw().toBuffer({ resolveWithObject: true });

  // 2. 新水着ダメージ顔画像 (1024x1024) を 200x200 にリサイズ
  ...
  await sharp(outBuf, {
    ...
    .webp({ quality: 90 })
    .toFile('public/assets/icons/icon_dragon_summer_damage.webp');

  console.log('Saved perfect icon_dragon_summer_damage.webp successfully');
}
