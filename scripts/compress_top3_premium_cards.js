...
async function optimizeTop3() {
  console.log(
    '--- プレミアムカード画像 TOP3 の軽量化（PNG -> WebP直接再エンコード）を開始 ---'
  );
  ...
  for (const item of targets) {
    ...
    const pageHeight =
      metadata.pageHeight || Math.round(metadata.height / totalPages);
    const delays = metadata.delay || Array(totalPages).fill(33);
    img.destroy();

    console.log(
      `[処理中] ${item.name} (${originalSizeMB} MB, ${totalPages} コマ)...`
    );

    // 1. 各フレームの抽出（GIFではなくフルカラー・透明度保持のPNG形式を使用）
    const tmpDir = path.join(
      'public/assets/cards',
      `_tmp_${item.name.replace('.webp', '')}`
    );
    ...
    if (fs.existsSync(optPath)) {
      ...
      const reduction = (
        ((srcBuf.length - optBuf.length) / srcBuf.length) *
        100
      ).toFixed(1);
      ...
      console.log(
        `   元サイズ: ${originalSizeMB} MB -> 軽量化後: ${newSizeMB} MB (削減率: -${reduction}%, 枠数: ${count}/${totalPages})`
      );
    }
