...
const SKILLS = {
    ...
    clone: { name: '分身', icon: '👥', desc: (val) => `自分のレーンに、自身と同じパワーの分身を${val || 1}体生成する。` },
    lone_wolf: { name: '単騎', icon: '🐺', desc: (val) => `場に出た時、自分の空いているレーンの数×${val || 3}だけパワーを上げる。` },
    soul_bind: { name: '魂縛', icon: '⛓️', desc: (val) => `戦闘で敵を破壊した時、パワーを+${val || 2}する。` },
    sturdy: { name: '頑丈', icon: '🛡', desc: (val) => '戦闘で受けるダメージを半減する（端数切り捨て）。' },
    berserk: { name: '狂乱', icon: '💢', desc: (val) => `【デメリット】自分の場の隣のレーンにいるカードに${val}ダメージ。` },
    ...
    hero: { name: '英雄', icon: '🏆', desc: (val) => `場に出た時、自身の埋まっているレーンにつきパワーを${val >= 0 ? '+' : ''}${val}する` },
