...
export const SKILLS = {
  ...
  dispel: {
    ...
    icon: '🔓',
    desc: (val) =>
      `召喚時、相手のカード${val || 1}枚を選択し、そのカードが装備中のカードを全て破壊する。対象が「装備」を持つカードならそのカードを破壊する。`,
  },
  invade: {
    name: '侵略',
    icon: '🛸',
  ...
  crush: {
    ...
    icon: '🔨',
    desc: (val) =>
      `召喚時、相手のカード${val || 1}枚を選択する。対象が「防御」を持つカードならそのカードを破壊する。`,
  },
  substitute: {
    name: '身替',
    icon: '🎭',
  ...
  explore: {
    ...
    icon: '🗺',
    desc: (val) =>
      `召喚時、デッキからパワー${val}以下のカードを1枚まで選択して手札に加える。その後、手札を1枚捨ててデッキをシャッフルする。`,
  },
  possession: {
    name: '憑依',
    icon: '💞',
