...
export const SKILLS = {
  ...
  bind: {
    ...
    icon: '🕸️',
    desc: (val) =>
      `召喚時、正面のカードを${val}ターンの間「スタン」状態（攻撃せず、敵カードや敵リーダーにダメージを与えられない）にする。`,
  },
  growth: {
    name: '成長',
    icon: '🌱',
    desc: (val) => `自分のターン開始時、パワー${val >= 0 ? '+' : ''}${val}。`,
  },
  intercept: {
    name: '迎撃',
    icon: '👁',
  ...
  stealth: {
    ...
    icon: '👣',
    desc: (val) =>
      `召喚時、自身を${val || 1}ターンの間「無敵」状態（戦闘ダメージを受けない）にする。`,
  },
  reflect: {
    name: '反射',
    icon: '↩️',
  ...
  */
  ...
  startup: {
    ...
    /**
     * 「起動」スキルの説明テキストを生成する。
     * 自身の上にカードを重ねた時、1回まで身代わりとして自身の「スタン」状態と「防御」をなくす。
     * @returns {string} スキル説明文
     */
    desc: () =>
      '1回まで自身の上にカードを配置するとき、代わりに自身の「スタン」状態と「防御」をなくす。(重ねたカードは墓地に送られる)',
  },
  immune: {
    name: '無効',
    icon: '🚫',
  ...
  dodge: {
    ...
    icon: '💠',
    desc: (val) =>
      `${val}以上のダメージを無効化する。（「スタン」状態の場合は無効）`,
  },
  fate: {
    name: '運命',
    icon: '🎲',
  ...
  phase: {
    ...
    icon: '🌫️',
    desc: () =>
      '「位相」か「防御」を持たないカードと戦闘を行わず、互いにリーダーを直接攻撃する。（「スタン」状態の場合は無効）',
  },
  petrify: {
    name: '石化',
    icon: '🗿',
  ...
  move: {
    ...
    icon: '🏃',
    desc: () =>
      '自分のターン開始時、隣のレーンに移動できる。（「スタン」状態の場合は無効）',
  },
  freeze: {
    name: '凍結',
    icon: '❄️',
    desc: (val) =>
      `召喚時、正面とその隣のカードを${val}ターンの間「スタン」状態（攻撃せず、敵カードや敵リーダーにダメージを与えられない）にする。`,
  },
  loss: {
    name: '喪失',
    icon: '🕳️',
  ...
  teleport: {
    ...
    icon: '🎩',
    desc: () =>
      '自分のターン開始時、ランダムな自分の空きレーンに移動する。（「スタン」状態の場合は無効）',
  },
  brutal: {
    name: '暴虐',
    icon: '🌪️',
  ...
  crush: {
    ...
    icon: '🔨',
    desc: () =>
      `召喚時、お互いの場の「防御」を持つカードと「スタン」状態のカードを全て破壊する。`,
  },
  treason: {
    name: '反逆',
    icon: '👑',
  ...
  unleash: {
    ...
    /**
     * 「解放」スキルの説明テキストを生成する。
     * 召喚時、自身の「スタン」状態と「防御」をなくす。
     * @returns {string} スキル説明文
     */
    desc: () => '召喚時、自身の「スタン」状態と「防御」をなくす。',
  },
  all_forms: {
    name: '万相',
    icon: '🎭',
  ...
  protection: {
     ...
     */
    desc: () =>
      '召喚時、自分の場のカード1体を選択し、次の自分のターン開始時まで「加護」状態（全てのダメージを受けず、破壊されない）にする。',
  },
};
