...
export const GAME_VERSION = '0.4.0.24';
...
export const DECKS_KEY = 'mini_card_battle_decks';
export const DECKS_BACKUP_KEY = 'mini_card_battle_decks_backup';
...
export const SE_MUTED_KEY = 'mini_card_battle_se_muted';
export const LEGACY_VOLUME_KEY = 'mini_card_battle_volume';
...
export const DEFENSE_EXCHANGE_LINEUP = freezeLineup([
  ...
  { id: 'bountyhunter', type: 'premium', cost: SILVER_PREMIUM_EXCHANGE_COST },
  { id: 'alraune', type: 'premium', cost: SILVER_PREMIUM_EXCHANGE_COST },
  { id: 'priestess', type: 'premium', cost: SILVER_PREMIUM_EXCHANGE_COST },
  { id: 'cavalry', type: 'premium', cost: SILVER_PREMIUM_EXCHANGE_COST },
 ...
 */
export const HIGH_DIFFICULTY_COSMETIC_CARD_IDS = Object.freeze([
  'scientist',
  'hellkite',
  'duelist',
  'cthulhu',
...
/**
 * 高難易度イベントの装飾品（プレイマット・アイコン）交換所アイテムを生成する
 * @returns {Array<Object>} 交換所装飾品アイテム配列
 */
function generateHighDifficultyCosmeticExchangeItems() {
  const cardMap = new Map(CARD_MASTER.map((c) => [c.id, c]));
  const items = [];

  // 1. プレイマット（PLAYMAT_EXCHANGE_COST）
  for (const cardId of HIGH_DIFFICULTY_COSMETIC_CARD_IDS) {
