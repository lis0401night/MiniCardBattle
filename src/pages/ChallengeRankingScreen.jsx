    ...
    const fetchPlayers = async () => {
      ...
      try {
        ...
        if (result.success) {
          ...
          // ランキングソート (試練の宮殿ポイントの降順)
          activePlayers.sort(
            (a, b) =>
              (b.challenge_total_points || b.challenge_points || 0) -
              (a.challenge_total_points || a.challenge_points || 0)
          );

          // 各プレイヤーに対する計算を追加
          activePlayers = activePlayers.map((p, index) => {
            const pTotalPoints =
              p.challenge_total_points || p.challenge_points || 0;
            return {
              ...p,
              rankIndex: index,
