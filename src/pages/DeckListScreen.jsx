...
import { initSelectScreen, confirmCharSelect, goBackFromSelect } from '../hooks/uiMainCore.js';
...
export default function DeckListScreen() {
    ...
    useEffect(() => {
        ...
        return () => { window.forceUpdateDeckList = null; };
    }, []);
    ...
    const handleDeleteDeck = (index) => {
        ...
        showConfirmModal?.("このデッキを削除しますか？\n（この操作は取り消せません）", () => {
            GameState.decks.splice(index, 1);
            if (GameState.currentDeckIndex >= index && GameState.currentDeckIndex > 0) {
    ...
    return (
        <div id="screen-deck-list" className="screen active" style={{
        ...
        >
                        ...
                        {pages.map((pageItems, pageIndex) => (
                            <div key={`page-${pageIndex}`} style={{ 
                                    ...
                                    if (item === 'create') {
                                        return (
                                            <button
                                                ...
                                                style={{ flexShrink: 0, borderColor: '#facc15', borderStyle: 'dashed', background: 'rgba(250, 204, 21, 0.1)', justifyContent: 'center' }}
                                                onClick={() => {
                                                    if (!isSwipingRef.current && !isDraggingRef.current) handleCreateNew();
                                                }}
                                            >
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', color: '#facc15', fontWeight: 'bold' }}>
                                    ...
                                    return (
                                        <div 
                                            ...
                                            style={{ 
                                                ...
                                                boxShadow: isHoveringThis ? `0 0 15px ${char.color}` : 'none'
                                            }}
                                            onPointerDown={(e) => handleBannerPointerDown(e, idx, 'deck')}
                                            onTouchStart={(e) => handleBannerPointerDown(e, idx, 'deck')}
                                        >
                                            <button
                                                className={`btn-banner no-transition`}
                                                style={{ margin: 0, borderColor: char.color, width: '100%', pointerEvents: dragIndex !== null ? 'none' : 'auto' }}
                                                onClick={(e) => {
                                                    // DnD中はクリック発火させない
                                                        ...
                                                        <img 
                                                            ...
                                                            onClick={(e) => {
                                                                ...
                                                                if (window.showCharDetailModal) {
                                                                    window.showCharDetailModal({ ...char, hideDecideButton: true, targetDeckIndex: idx });
                                                                }
                                                            }}
                                                        />
                                ...
                                {Array.from({ length: itemsPerPage - pageItems.length }).map((_, i) => (
                                    <div key={`dummy-${pageIndex}-${i}`} style={{ height: '80px', width: '100%', flexShrink: 0, visibility: 'hidden' }}></div>
                                ))}
                            </div>
                        ))}
                    </div>
            ...
            {dragIndex !== null && (() => {
                ...
                return (
                    ...
                    >
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                            <img src={getSkinImage ? getSkinImage(char, deck.playerSkins?.[char.id], 'icon') : char.icon} className="banner-icon" alt="" style={{ width: '60px', height: '60px', marginRight: '15px' }} />
                            <span className="banner-text" style={{ color: char.color, fontSize: '1.2rem', fontWeight: 'bold' }}>
