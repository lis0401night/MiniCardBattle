...
export default function GlobalModals() {
  ...
  useEffect(() => {
    ...
    window.showExchangeDetailModal = (data) => {
      playSound?.(SOUNDS?.seClick);
      setExchangeDetailData(data);
    };

    window.closeExchangeDetailModal = () => {
      playSound?.(SOUNDS?.seClick);
      setExchangeDetailData(null);
    };
    ...
    return (
      <div className={`preview-content ${styleProps.containerClass || ''}`} style={{ margin: styleProps.margin, cursor: 'default', borderColor: styleProps.borderColor, boxShadow: styleProps.boxShadow }} onClick={(e) => e.stopPropagation()}>
         ...
         <div className="preview-details">
            ...
            <h2 style={{ color: nameColor, marginTop: showPremiumTag ? '5px' : '0' }}>{card.name}</h2>
            ...
            <div className="preview-scroll-area">
               ...
               {card.flavor && <p className="preview-flavor-text" style={{ display: 'block' }}>{card.flavor}</p>}
            </div>
  ...
  return (
    <>
      ...
      {enemyDeckData && (
        <div className="modal-overlay" style={{ zIndex: 2000, display: 'flex' }} onClick={closeEnemyDeckModal}>
          <div className="skill-modal-box modal-pop-animation" style={{ width: '95%', maxWidth: '440px', padding: '20px' }} onClick={(e) => e.stopPropagation()}>
            ...
            <button className="btn" style={{ marginTop: '20px', width: '100%' }} onClick={closeEnemyDeckModal}>閉じる</button>
          </div>
        </div>
      )}
      ...
      {/* Card Preview Modal */}
      {cardPreviewData && (
        <div className="modal-overlay" style={{ zIndex: 2100, display: 'flex' }} onClick={handleCloseCardPreview}>
           {renderCardPreviewContent(cardPreviewData.card, { showPreviewActions: true })}
        </div>
      )}
      ...
      {charDetailData && (
        <div className="screen" style={{ background: 'rgba(0,0,0,0.85)', zIndex: 50, display: 'flex', position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: 'var(--panel-bg, #1e293b)', border: '2px solid #facc15', borderRadius: '12px', padding: '20px', width: '90%', maxWidth: '350px', display: 'flex', flexDirection: 'column', alignItems: 'center', boxShadow: '0 0 30px rgba(0,0,0,0.8)' }}>
                <img src={charDetailData.image} style={{ width: '160px', height: '200px', objectFit: 'cover', borderRadius: '8px', border: '2px solid #334155', marginBottom: '15px' }} alt={charDetailData.name} />
                ...
                {charDetailData.leaderSkill && (
                    <div style={{ background: 'rgba(0,0,0,0.5)', padding: '10px', borderRadius: '8px', width: '100%', boxSizing: 'border-box', marginBottom: '20px', border: '1px solid #475569' }}>
      ...
      {/* Exchange Detail Modal (Perfectly restored from BaseUI.js) */}
      {exchangeDetailData && (
        <div className="screen" style={{ background: 'rgba(0,0,0,0.85)', zIndex: 50, display: 'flex', position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center' }}>
            <div className="preview-content" style={{ position: 'relative', width: '90%', maxWidth: '350px' }}>
                <div id="exchange-detail-card-container">
                    <div className={`card blue${exchangeDetailData.itemObj.rarity ? ` rarity-${exchangeDetailData.itemObj.rarity}` : ''}`} style={{ width: '120px', height: '180px', position: 'relative', display: 'block', transform: 'scale(1.2)', marginTop: '10px', marginBottom: '10px' }}>
                        <div className="card-bg" style={{ backgroundImage: `url('${exchangeDetailData.type === 'premium' ? getCardImgUrl(exchangeDetailData.itemObj).replace('.jpg', '_premium.gif') : getCardImgUrl(exchangeDetailData.itemObj)}')` }}></div>
                        <div className="card-power" style={{ fontSize: '2rem', bottom: 0, right: '6px' }}>{exchangeDetailData.itemObj.power}</div>
                        {window.renderSkillTag && <div dangerouslySetInnerHTML={{ __html: window.renderSkillTag(exchangeDetailData.itemObj) }}></div>}
                    </div>
                </div>
                
                <div className="preview-details">
                    <h2 id="exchange-detail-name">
                        {exchangeDetailData.type === 'premium' ? `${exchangeDetailData.itemObj.name} (プレミアム)` : `${exchangeDetailData.itemObj.name} (カード)`}
                    </h2>
                    
                    <div className="preview-scroll-area">
                        <div id="exchange-detail-skills-list" className="preview-skills-list">
                            {(() => {
                                let skillCandidates = [];
                                if (exchangeDetailData.itemObj.skill && exchangeDetailData.itemObj.skill !== 'none') {
                                    skillCandidates.push({ id: exchangeDetailData.itemObj.skill, value: exchangeDetailData.itemObj.skillValue });
                                }
                                if (Array.isArray(exchangeDetailData.itemObj.skills)) {
                                    skillCandidates = skillCandidates.concat(exchangeDetailData.itemObj.skills);
                                }
                                if (skillCandidates.length === 0) return <p className="preview-skill-desc">能力なし</p>;

                                return skillCandidates.map((sk, idx) => {
                                    const s = SKILLS[sk.id];
                                    if (!s) return null;
                                    const val = (sk.value === null || sk.value === undefined) ? '' : sk.value;
                                    const desc = typeof s.desc === 'function' ? s.desc(sk.value) : s.desc;
                                    
                                    // チョイススキル等のアコーディオンは省略しシンプル表示（uiMainCore.jsの実装を元に）
                                    return (
                                        <div className="preview-skill-item" key={idx}>
                                            <div className="preview-skill-badge">{s.icon} {s.name}{val}</div>
                                            <p className="preview-skill-desc">{desc}</p>
                                        </div>
                                    );
                        ...
                        <p id="exchange-detail-flavor" className="preview-flavor-text">{exchangeDetailData.itemObj.flavor || '...'}</p>
                    </div>
                    ...
                    <div style={{ background: 'rgba(0,0,0,0.5)', padding: '5px', borderRadius: '8px', width: '100%', boxSizing: 'border-box', marginTop: '10px', border: '1px solid #475569', textAlign: 'center' }}>
                        ...
                        <div id="exchange-detail-cost" style={{ fontSize: '1.2rem', color: '#10b981', fontWeight: 'bold' }}>{exchangeDetailData.cost} pt</div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '10px', width: '100%', marginTop: '10px', flexShrink: 0 }}>
                        ...
                        <button 
                             id="btn-exchange-confirm" 
                             className="btn" 
                             style={{ flex: 1, minHeight: '40px', padding: '5px', background: exchangeDetailData.isMaxed || !exchangeDetailData.canExchange ? '#475569' : 'linear-gradient(45deg, #f97316, #ea580c)', color: exchangeDetailData.isMaxed || !exchangeDetailData.canExchange ? '#94a3b8' : '#ffffff', marginTop: 0, fontSize: '0.9rem' }} 
                             onClick={() => {
                                if (exchangeDetailData.isMaxed) {
                                    showAlertModal?.(exchangeDetailData.type === 'premium' ? "既にプレミアム化済みです。" : "所持上限(4枚)に達しています。");
                                } else if (!exchangeDetailData.canExchange) {
                        ...
                        >
                            {exchangeDetailData.isMaxed ? '交換済み' : (!exchangeDetailData.canExchange ? 'ポイント不足' : '交換')}
                        </button>
                    </div>
                </div>
            </div>
      ...
      {/* Skill Confirm Modal */}
      {skillConfirmData && (
        <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.85)', zIndex: 60, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px', boxSizing: 'border-box' }} onClick={window.closeSkillConfirmModalReact}>
