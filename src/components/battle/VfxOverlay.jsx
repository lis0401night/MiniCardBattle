...
function VfxItem({ effect }) {
    ...
    const getBaseTop = () => {
        const base = dynamicPos ? dynamicPos.top : (() => {
    ...
    const positionClass = (data.position === 'fill' && dynamicPos) ? 'fill-dynamic' : (data.position || '');

    return (
        <div className={`vfx-overlay ${type} ${side} ${positionClass}`} style={{ '--vfx-top': `${finalTop}%`, '--vfx-left': `${finalLeft}%` }}>
            <div 
                ...
                style={{ 
                    ...
                    top: 'var(--vfx-top)', 
                    left: 'var(--vfx-left)', 
                    width: (data.position === 'lane' || (data.position === 'fill' && dynamicPos)) ? '33.3%' : '100%', 
