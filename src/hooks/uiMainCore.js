...
import { initEventSatanMode, initEventAndroidHighMode, initEventDragonHighMode, initEventKnightHighMode, initEventCthulhuHighMode, initEventElfHighMode, initEventClericHighMode, initEventDevilhunterHighMode, initEventWitchHighMode, initEventOniHighMode, loadPlayerDeck } from './events.js';
...
export function incrementRulesClickCount() { return ++rulesClickCount; }
export function resetRulesClickCount() { rulesClickCount = 0; }
...
export async function startAttackBattle(enemyPlayerData) {
    ...
    try {
        ...
        // 敵の設定を保存
        GameState.enemyConfig = { ...CHARACTERS[enemyPlayerData.character] || CHARACTERS.android };
        ...
        GameState.enemyConfig.total_points = enemyPlayerData.total_points || enemyPlayerData.points || 0;
...
export function setRenderExchangeHook(h) { renderExchangeHook = h; }
...
export function showExchangeDetail(id, type, cost, itemObj, canExchange, isMaxed) {
    playSound(SOUNDS.seClick);
    pendingExchange = { id, type, cost };
    if (window.showExchangeDetailModal) {
        window.showExchangeDetailModal({ id, type, cost, itemObj, canExchange, isMaxed });
    }
}
...
export function confirmExchange() {
    ...
    if (type === 'premium') {
        if (!GameState.unlockedPremiumCards.includes(id)) GameState.unlockedPremiumCards.push(id);
        ...
        localStorage.setItem('mini_card_battle_inventory', JSON.stringify(GameState.playerInventory));
        playSound(SOUNDS.seSkill);
...
export function openEnemyDeckPreview(level) {
    ...
    if (!deckIds || deckIds.length === 0) {
        if (window.showAlertModalHook) window.showAlertModalHook("該難易度のデッキデータが空です。");
        return;
    }
...
export function setCloseEnemyDeckModalHook(h) { closeEnemyDeckModalHook = h; }
