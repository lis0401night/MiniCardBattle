...
import { getDialogue, playSound, stopAllBGM, switchScreen } from '../utils/gameUtils.js';
...
export function initCampaignMode() {
    ...
    if (!saveData) {
        ...
        localStorage.setItem('mini_card_battle_campaign_save', JSON.stringify(saveData));
    }
...
export function startCampaignNode(nodeId) {
    ...
    if (node) {
        ...
        if (node.enemyConfig) GameState.enemyConfig = typeof node.enemyConfig === 'function' ? node.enemyConfig() : node.enemyConfig;
        if (node.dialogueQueue) GameState.dialogueQueue = node.dialogueQueue;

        if (node.appState === 'pre_dialogue' || node.appState === 'ending_dialogue') {
            setupDialogueScreen();
        } else {
...
export function onCampaignDialogueEnd() {
    ...
    if (currentNodeData?.appState === 'pre_dialogue' || currentNodeData?.appState === 'ending_dialogue') {
