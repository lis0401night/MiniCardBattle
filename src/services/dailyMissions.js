/**
 * デイリーミッション進捗・報酬受取管理サービス
 *
 * 防衛戦・試練の宮殿・夢幻の闘技祭での勝利記録、日本標準時（JST 00:00）日次自動リセット、
 * パックvol01（vol01:ビギニング）の報酬付与および達成状況判定を提供します。
 * 定数定義（DAILY_MISSIONS）は constants 層から参照し、依存関係を一方向に保ちます。
 */
...
import {
  checkIsDungeonMode,
  checkIsTournamentMode,
} from '../utils/gameUtils.js';
import { INVENTORY_KEY, MAX_CARD_COPIES } from '../utils/constants/config.js';
import {
  PACK_MASTER,
  PACK_VOL01_CARD_IDS,
  drawCardFromPack,
 ...
 * @returns {{ date: string, missions: Record<string, { progress: number, isClaimed: boolean }> }} 初期データオブジェクト
 */
 ...
 *
 * @returns {{ date: string, missions: Record<string, { progress: number, isClaimed: boolean }> }} 進捗データオブジェクト
 */
 ...
 *
 * @param {{ date: string, missions: Record<string, { progress: number, isClaimed: boolean }> }} data - 保存する進捗データ
 */
 ...
 * @returns {{ success: boolean, cardId?: string, packDef?: Object, error?: string }} 抽選結果オブジェクト
 */
